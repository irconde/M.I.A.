...
watcher
  .on('add', async (file) => {
      console.log('File', path.resolve(file), 'has been added');
      if (validateRegExp(getFileName(file), regExpDCS)){
          console.log('file is of type .dcs');
          if (validateRegExp(getFileName(file), regExpFileName)) {
              console.log('file is formatted as num_img.dcs');
            ...
            // Rename the file
            fs.rename(file, `${imgPath}${maxFileNumber+1}_img.dcs`, (err) => console.log(err));
            console.log(`File Not Formated Properly --- Renamed file to ${imgPath}${maxFileNumber+1}_img.dcs`)
          }
      } else {
          console.log('file is not of type .dcs');
      }     
        console.log(fileQueue);
    })
  .on('change', async (file) => {
      console.log('File', path.resolve(file), 'has been changed');
    })
  .on('unlink', async (file) => {
      console.log('File', path.resolve(file), 'has been removed');
...
router.get("/next", (req, res) => {
    console.log('/next image serve request')
    if (fileQueue.length === 0){
...
io.on("connection", (socket) => {
    console.log('New Client Connected to File Server');
    
    socket.on("disconnect", () => {
        console.log("Client disconnected from File Server");
    })
...
async function storeFile(data) {
    ...
    fs.writeFile(fileNameDir, data, function(err) {
        ...
        console.log(`Saving Unvalidated DICOS-TDR Image...`);
    })
}
